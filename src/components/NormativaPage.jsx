    ...
    <div style={{padding:"20px 24px 40px"}}>
      ...
      <div style={{display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:12}}>
        <div>
          ...
          <div style={{fontSize:15, color:t.textSec, fontFamily:"'Rajdhani',sans-serif"}}>Configurazione limiti normativi e target operativi — D.Lgs 152/2006 Allegato 5</div>
        </div>
        <div style={{display:"flex", gap:10, alignItems:"flex-end", flexWrap:"wrap"}}>
          <select value={tipoScarico} onChange={e => handleTipoChange(e.target.value)}
            style={{background:t.surface, border:`1px solid ${t.border}`, color:t.text, padding:"8px 14px", borderRadius:7, fontFamily:"'Rajdhani',sans-serif", fontSize:14, cursor:"pointer"}}>
          ...
          <button onClick={() => handleTipoChange(tipoScarico)}
            style={{background:t.surface2, border:`1px solid ${t.border}`, color:t.textSec, padding:"8px 16px", borderRadius:7, fontFamily:"'Rajdhani',sans-serif", fontWeight:600, fontSize:14, cursor:"pointer"}}>↺ Default</button>
          <button onClick={() => { setSaved(true); setTimeout(() => setSaved(false), 2500); }}
            style={{background:saved?`${t.green}18`:`${t.accent}18`, border:`1px solid ${saved?t.green:t.accent}`, color:saved?t.green:t.accent, padding:"8px 22px", borderRadius:7, fontFamily:"'Rajdhani',sans-serif", fontWeight:700, fontSize:14, cursor:"pointer", transition:"all 0.3s"}}>
            {saved ? "✓ Salvato" : "💾 Salva"}
          </button>
        </div>
      </div>

      <div style={{background:`${t.accent}0D`, border:`1px solid ${t.accent}30`, borderRadius:10, padding:"14px 20px", marginBottom:20, display:"flex", gap:14, alignItems:"flex-start"}}>
        <span style={{fontSize:24, flexShrink:0}}>⚖️</span>
        <div style={{fontSize:14, color:t.textSec, fontFamily:"'Rajdhani',sans-serif", lineHeight:1.6}}>
          <strong style={{color:t.accent}}>D.Lgs 152/2006 — {tipoScarico}</strong><br/>
          Clicca su qualsiasi valore <span style={{fontFamily:"'Share Tech Mono',monospace", background:t.surface2, padding:"2px 6px", borderRadius:3}}>✎</span> per modificarlo.
        </div>
      </div>
      ...
      {norms.map((cat, ci) => (
        <div key={cat.categoria} style={{marginBottom:16}}>
          <button onClick={() => setExpandedCat(expandedCat === ci ? null : ci)}
            style={{width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, padding:"12px 16px", background:t.surface, border:`1px solid ${t.border}`, borderLeft:`4px solid ${cat.colore}`, borderRadius:expandedCat===ci?"10px 10px 0 0":"10px", cursor:"pointer"}}>
            <div style={{display:"flex", alignItems:"center", gap:12}}>
              <div style={{fontFamily:"'Rajdhani',sans-serif", fontWeight:700, fontSize:16, color:t.text}}>{cat.categoria}</div>
              <Tag color={cat.colore}>{cat.params.length} param.</Tag>
            </div>
            <span style={{color:t.textMuted, fontSize:14, transform:expandedCat===ci?"rotate(180deg)":"rotate(0deg)", transition:"transform 0.2s"}}>▼</span>
          </button>
          {expandedCat === ci && (
                ...
                <tbody>
                  {cat.params.map((p, pi) => (
                    <tr key={p.id} style={{background:pi%2===0?t.surface:`${t.surface2}88`}}>
                      <td style={{...tdS, fontWeight:700, color:t.text, fontSize:15}}>{p.nome}</td>
                      <td style={{...tdS, fontFamily:"'Share Tech Mono',monospace", fontSize:13, color:t.textSec}}>{p.unita||"—"}</td>
                      <td style={{...tdS, fontFamily:"'Share Tech Mono',monospace", fontSize:13, color:t.textMuted}}>{p.rif}</td>
                      ...
                      <td style={{...tdS, color:t.green, fontFamily:"'Share Tech Mono',monospace", fontSize:13, fontWeight:700}}>✓ Attivo</td>
                    </tr>
                  ))}
                </tbody>
              </table>
